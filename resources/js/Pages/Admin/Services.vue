    ...
    <AdminLayout>
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                ...
                <h1 class="text-3xl sm:text-4xl font-black text-white tracking-tight">Services</h1>
            </div>
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-3 mb-2">
                            ...
                            <span class="text-xs px-2 py-0.5 rounded-full"
                                  :class="service.is_active ? 'bg-senzu/10 text-senzu' : 'bg-red-500/10 text-red-400'">
                                {{ service.is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
                        <p class="text-gray-400 text-sm mb-2">{{ service.description }}</p>
                        <div v-if="service.bullet_points" class="flex flex-wrap gap-2">
                            <span v-for="(bp, i) in service.bullet_points" :key="i"
                                  class="text-xs bg-dark-bg border border-dark-border rounded px-2 py-1 text-gray-400">
                                {{ bp }}
                            </span>
                        </div>
                    </div>
                    <div class="flex gap-2 shrink-0 self-end sm:self-auto">
                        <button @click="openEdit(service)" class="text-gray-400 hover:text-senzu transition-colors p-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </button>
                        <button @click="deleteService(service)" class="text-gray-400 hover:text-red-400 transition-colors p-2">
        ...
        <teleport to="body">
            <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4" @click.self="showModal = false">
                ...
                <div class="relative bg-dark-card border border-dark-border rounded-2xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                    <h3 class="text-white font-bold text-lg mb-4">{{ editing ? 'Edit' : 'Add' }} Service</h3>
                    <form @submit.prevent="submitForm" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea v-model="form.description" rows="3" required class="w-full bg-dark-bg border border-dark-border rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-senzu/50 transition-colors resize-none"></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea v-model="bulletText" rows="4" class="w-full bg-dark-bg border border-dark-border rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-senzu/50 transition-colors resize-none"></textarea>
                        </div>
                        <div class="flex items-center gap-2">
                            <input v-model="form.is_active" type="checkbox" id="active" class="w-4 h-4 rounded border-dark-border bg-dark-bg text-senzu">
                            <label for="active" class="text-gray-400 text-sm">Active</label>
                        </div>
...
function submitForm() {
    form.bullet_points = bulletText.value.split('\n').filter(l => l.trim());
    if (editing.value) {
        form.put(`/admin/services/${editing.value.id}`, {
            preserveScroll: true,
